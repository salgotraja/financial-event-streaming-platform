...
/**
 * Paces {@link TradeGenerator} into {@link TradeEventPublisher} at a configured rate (FR-01.1).
 *
 * <p><strong>Batched, not one event per wake.</strong> The JVM cannot reliably park for less than a
 * millisecond, so a driver that emitted one event per wake would top out near 1,000 events/sec, a
 * fraction of a realistic target rate. The driver instead wakes on a fixed interval and emits the
 ...
 *
 * <p>A missed deadline is dropped rather than made up. Bursting to catch up after a stall would push
 * a spike into a broker that has already shown it cannot keep pace.
 *
 ...
 */
public class TradeGenerationDriver implements SmartLifecycle {

    private static final Logger log = LoggerFactory.getLogger(TradeGenerationDriver.class);
    ...
    public TradeGenerationDriver(TradeGenerator generator,
    ...
    public void start() {
        ...
        try {
            ...
            running = true;
            worker = Thread.ofVirtual().name("trade-generator").start(this::run);
            log.info("Trade generation started ratePerSecond={} batchSize={} batchIntervalMs={}",
    ...
    /** Trades published since start. Exposed for tests and for a throughput probe. */
    public long emitted() {
        return emitted.get();
    }

    /** Trades emitted per wake, derived from the target rate and the wake interval. */
    int batchSize() {
        return batchSize;
    }
